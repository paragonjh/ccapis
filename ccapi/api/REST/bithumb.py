...
# Import Homebrew
from ccapi.api.rest import RESTAPIClient
...
class BithumbREST(RESTAPIClient):
    ...
    def sign(self, url, endpoint, endpoint_path, method_verb, *args, **kwargs):
        ...
        try:
            __url = dict(endpoint, **kwargs['params'])
        except KeyError:
            __url = endpoint

        __nonce = self.nonce()
        data = endpoint+ chr(0) + urllib.parse.urlencode(__url) + chr(0) + __nonce
    ...
    # };
    #
    # url_array = dict(endpoint_item_array, **rgParams);  # Concatenate the two arrays.
    #
